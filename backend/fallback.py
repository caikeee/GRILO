...
def get_fallback_response(
    ...
    if mode == "shadow":
        phrases = SHADOW_FALLBACK_PHRASES.get(level or "b1", SHADOW_FALLBACK_PHRASES["b1"])
        fallback_phrase = phrases[0]  # Usar primeira frase como fallback
        return {
            "response": fallback_phrase,
            "fallback": True,
            "error_scenario": scenario,
            "retry_after_ms": 2000 if scenario == RATE_LIMIT else 1000,
        }

    elif mode == "dictation":
        fallback_text = DICTATION_FALLBACK.get(level or "b1", DICTATION_FALLBACK["b1"])
        return {
        ...
        # Free ou Guided mode
        responses = FALLBACK_RESPONSES.get(mode, FALLBACK_RESPONSES["free"])
        fallback_msg = responses.get(scenario, responses["unknown"])

        return {
            "response": fallback_msg,
            "fallback": True,
            "error_scenario": scenario,
            "retry_after_ms": 5000 if scenario == RATE_LIMIT else 3000,
        }
...
def log_fallback_usage(
    ...
    # user_id sempre 0 hoje — chamadores não propagam usuário real
    logger.warning(
